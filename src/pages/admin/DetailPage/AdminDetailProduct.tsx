...
import type { UpdateProductCommand } from "@/models/Product";
...
const AdminDetailProduct = observer(() => {
  ...
  const { productStore, categoryStore, commonStore, attributeValueStore } =
    useStore();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(true);

  // Form state (same shape as CreateProductCommand)
  const [formData, setFormData] = useState<UpdateProductCommand>({
    name: "",
    sku: "",
    slug: "",
    shortDescription: "",
  ...
  // Other UI state reused from AddProduct
  ...
  const mapProductAttributes = (
    productAttrs: ProductAttributeDto[],
    allAttributes: Attribute[],
  ) => {
    return productAttrs
      .map((attr) => {
        const attrId = (attr as { attributeId?: number }).attributeId;
        const matched = attrId
          ? allAttributes.find((a) => a.id === attrId)
          : allAttributes.find((a) => a.code === attr.code);
        ...
        return {
          ...
          value: attr.value || "",
          extraData: (attr as { extraData?: string }).extraData || "",
          isOpen: false,
        };
      })
      .filter((item): item is {
    ...
    const load = async () => {
      ...
      if (p) {
        setFormData((prev) => ({
          ...
          basePrice: p.originalPrice ?? 0,
          salePrice: p.price ?? 0,
          salesUnit: p.salesUnit ?? prev.salesUnit,
          priceUnit: p.priceUnit ?? prev.priceUnit,
          conversionFactor: p.conversionFactor ?? prev.conversionFactor,
          categoryId: (p as any).categoryIds?.[0] ?? (p as any).categoryId ?? 0,
          categoryIds:
            (p as any).categoryIds ??
            ((p as any).categoryId ? [(p as any).categoryId] : []),
          weight: p.weight ?? 0,
          thumbnailUrl: p.thumbnail ?? prev.thumbnailUrl,
          imageIds: p.images
            ? p.images.map((i: any) => i.id).filter(Boolean)
            : [],
          specificationsJson: p.specificationsJson ?? "",
          isActive: true,
          tagIds:
            (p as { tagIds?: number[] }).tagIds ??
        ...
        if (p.images) setProductImages(p.images as AppImageDto[]);
      }
      setIsLoading(false);
    };
  ...
  useEffect(() => {
    ...
    setSelectedAttributes(mapProductAttributes(p.attributes, attributes));
  }, [productStore.selectedProduct, attributes]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await productStore.updateProduct(id, command);
      commonStore.showSuccess("Cập nhật sản phẩm thành công!");
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">Chi tiết / Cập nhật sản phẩm</h1>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-4">Đường dẫn</h2>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-1/2">ankhanhstore.com/san-pham/</span>
                    <input
                      type="text"
                      name="slug"
                      value={formData.slug}
                  ...
                  <div className="border border-gray-200 rounded-lg">
                    <div className="flex border-b border-gray-200 text-sm">
                      <button
                        ...
                        className={`flex-1 px-3 py-2 text-center ${categoryTab === "all" ? "bg-white font-medium text-gray-900 border-b-2 border-primary-500" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
                      >
                        Tất cả danh mục
                      </button>
                      <button
                        ...
                        className={`flex-1 px-3 py-2 text-center ${categoryTab === "popular" ? "bg-white font-medium text-gray-900 border-b-2 border-primary-500" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
                      >
                        Dùng nhiều nhất
                      </button>
                    </div>
