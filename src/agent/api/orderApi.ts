...
export const getOrdersOfUser = async (params: OrderParams): Promise<PaginatedResult<OrderSummaryDto[]>> => {
    ...
    // 2. Gọi API
    const response = await apiClient.getWithHeaders<any>(`/Orders/user?${queryParams.toString()}`)
...
export const getOrderById = async (orderNumber: string): Promise<OrderDto> => {
  const response = await apiClient.get<ApiResponse<OrderDto> | OrderDto>(`/Orders/${orderNumber}`)
